...
def insert_image_data(
        frame_count, capture_timestamp, raw_image_path, processed_image_path, width, height,
    brightness, blur_score, raw_size_kb, processed_size_kb):

    cursor.execute('''
    INSERT INTO image_data(frame_count,capture_timestamp,raw_image_path,processed_image_path,width,height
    brightness,blur_score,raw_size_kb,processed_size_kb)
       values(?,?,?,?,?,?,?,?,?,?)
    ''',
                   (frame_count,capture_timestamp,raw_image_path,processed_image_path,width,height,
...
def create_image_database():
    ...
    cursor.execute(
        ''' CREATE TABLE IF NOT EXISTS image_data( id INTEGER PRIMARY KEY AUTOINCREMENT, frame_count INTEGER, capture_timestamp TEXT, raw_image_path TEXT, processed_image_path TEXT, width INTEGER, height INTEGER, brightness REAL, blur_score REAL, raw_size_kb REAL, processed_size_kb REAL ) ''')
    connection.commit()
